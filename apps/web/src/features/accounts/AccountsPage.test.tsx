...
describe('AccountsPage replacement flows', () => {
  ...
  it('keeps a retained Antigravity matrix visible with error-tone bars after refresh failure', async () => {
    ...
    const quotaBars = card.findAll(
      (node) =>
        ...
        typeof node.props.className === 'string' &&
        node.props.className.includes('quotaBar') &&
        node.parent?.props['aria-label'] !== 'auth_cost.remaining'
    );
  ...
  it('loads quota history without automatically refreshing provider quota', async () => {
    ...
    expect(mocks.apiRequest.mock.calls.map(([call]) => call?.url)).toEqual([
    ...
    expect(mocks.getAccountWindowUsage).toHaveBeenCalledTimes(3);
    expect(
      mocks.apiRequest.mock.calls.every(([call]) => call?.url === CODEX_RATE_LIMIT_RESET_CREDITS_URL)
    ).toBe(true);
  });
  ...
  it('loads each reset-credit expiry on opening quota and preserves the cooldown timestamp', async () => {
        ...
        credits: [
          {
            ...
            expires_at: new Date(resetCreditExpiresAtMs + 86_400_000).toISOString(),
          },
          {
            id: 'reset-credit-1',
            status: 'available',
            reset_type: 'codex_rate_limits',
            expires_at: new Date(resetCreditExpiresAtMs).toISOString(),
          },
        ],
      },
      bodyText: '',
    ...
    expect(
      readText(renderer.root.findByProps({ 'data-quota-reset-credit-expiry': 'reset-credit-2' }))
    ).toContain(formatQuotaResetTimestamp(resetCreditExpiresAtMs + 86_400_000, 'en'));
    expect(
      renderer.root
        .findAll((node) => node.props['data-quota-reset-credit-expiry'])
        .map((node) => node.props['data-quota-reset-credit-expiry'])
    ).toEqual(['reset-credit-1', 'reset-credit-2']);
    expect(renderer.root.findByProps({ 'data-quota-reset-expiry-unknown': 'true' })).toBeDefined();
    expect(
  ...
  it('releases the reset lock when confirmation is cancelled so the credential can retry', async () => {
    ...
    expect(resetCalls).toHaveLength(3); // Tab opening, then two fresh reset verifications.
  ...
  describe('responsive quota presentation and split codex refresh flows', () => {
    ...
    it('creates minimal reset-detail state without marking quota inventory fetched on success or failure', async () => {
      mocks.location = { pathname: '/accounts', search: '?layout=grid' };
      ...
      // Case B: detail errors remain visible even without prior quota inventory.
      ...
      const failureState = (mocks.quotaState.codexQuota as Record<string, CodexQuotaState>)[
        failStoreKey
      ];
      expect(failureState.rateLimitResetCreditsError).toBe('upstream unavailable');
      expect(failureState.windows).toEqual([]);
      expect(failureState.fetchedAtMs).toBeUndefined();
      expect(failureState.quotaInventoryObserved).toBeUndefined();
    });
