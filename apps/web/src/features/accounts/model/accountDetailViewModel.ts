...
import { isConfirmedPaidXaiPlan, summarizeGroupedQuotaAvailability } from './accountQuotaSummary';
import {
...
export interface AccountDetailViewModel {
  ...
  quota: {
    ...
    resetCreditsApplicableAvailableCount: number | null;
...
export const buildAccountDetailViewModel = (
  ...
  return {
    ...
    quota: {
      ...
      resetCreditsApplicableAvailableCount:
        options.codexQuota?.rateLimitResetCreditsApplicableAvailableCount ?? null,
      resetCreditsError: options.codexQuota?.rateLimitResetCreditsError ?? null,
      resetCreditExpiries: [
        ...getSortedCodexResetCreditExpiries(options.codexQuota?.rateLimitResetCredits).map(
          (item) => ({ id: item.id, expiresAtMs: item.expiresAtMs })
        ),
        ...(options.codexQuota?.rateLimitResetCredits ?? []).flatMap((credit, index) =>
          credit.status === 'available' && !Number.isFinite(Date.parse(credit.expiresAt))
            ? [{ id: String(credit.id || index), expiresAtMs: null }]
            : []
        ),
      ],
    },
