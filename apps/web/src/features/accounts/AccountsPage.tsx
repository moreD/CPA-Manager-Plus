...
export function AccountsPage() {
  ...
  const codexResetCreditDetailOpenKeyRef = useRef('');
  ...
  const synchronizeExternalCredentialMutationMarkers = useCallback(
    ...
    [connectionFingerprint, credentialEvidenceScopeKey, files, reloadInspectionCredentialArtifacts]
  );
  ...
  useEffect(() => {
    if (detailTab !== 'quota' || selectedRow?.provider !== CODEX_CONFIG.type) {
      codexResetCreditDetailOpenKeyRef.current = '';
      return;
    }
    const openKey = JSON.stringify([connectionFingerprint, selectedRow.selectionKey]);
    if (codexResetCreditDetailOpenKeyRef.current === openKey) return;
    codexResetCreditDetailOpenKeyRef.current = openKey;
    void loadCodexResetCreditDetails(selectedRow);
  }, [connectionFingerprint, detailTab, loadCodexResetCreditDetails, selectedRow]);
